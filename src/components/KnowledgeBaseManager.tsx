...
const KnowledgeBaseManager: React.FC<KnowledgeBaseManagerProps> = ({ 
  ...
  const [isReadingFile, setIsReadingFile] = useState(false); // Mantido apenas para feedback visual momentâneo
    ...
    <>
        ...
        {isCreatingGroup ? (
          <div className="space-y-2">
            <input
              ...
              className="w-full h-8 py-1 px-2.5 border border-gray-300 dark:border-[rgba(255,255,255,0.1)] bg-gray-100 dark:bg-[#2C2C2C] text-gray-800 dark:text-[#E2E2E2] rounded-lg text-sm"
              onKeyPress={(e) => e.key === 'Enter' && handleCreateGroup()}
              autoFocus
            />
            <div className="flex items-center justify-end gap-2">
              <button onClick={handleCancelCreate} className="px-2.5 py-1 text-xs ...">Cancelar</button>
              <button onClick={handleCreateGroup} className="px-2.5 py-1 text-xs ...">Criar</button>
            </div>
          </div>
        ) : (
          <div className="relative w-full">
            <select
              ...
              className="w-full py-2 pl-3 pr-8 ... text-sm"
            >
              {knowledgeGroups.map(group => (
      ...
      <div className="flex items-center gap-2 mb-3">
        <input
          ...
          className="flex-grow h-8 py-1 px-2.5 ... text-sm"
          onKeyPress={(e) => e.key === 'Enter' && handleAddUrl()}
          disabled={sources.length >= maxSources || isReadingFile || isCreatingGroup}
        />
        <button onClick={handleAddUrl} disabled={sources.length >= maxSources || isReadingFile || isCreatingGroup} className="h-8 w-8 ..."><Plus size={16} /></button>
        <input type="file" ref={fileInputRef} onChange={handleFileChange} accept=".pdf,.txt,.md,text/plain,text/markdown,application/pdf" className="hidden" aria-hidden="true" />
        <button onClick={handleUploadClick} disabled={sources.length >= maxSources || isReadingFile || isCreatingGroup} className="h-8 w-8 ...">
          {isReadingFile ? <div className="w-4 h-4 ... animate-spin"></div> : <Upload size={16} />}
        </button>
      </div>
      
      {isReadingFile && ( <p className="text-xs ...">Processando arquivo...</p> )}
      {successMessage && ( <div className="..."><CheckCircle size={16} /><span>{successMessage}</span></div> )}
      {error && ( <div className="..."><AlertCircle size={16} /><span>{error}</span></div> )}
      {sources.length >= maxSources && ( <div className="..."><AlertCircle size={16} /><span>Máximo de {maxSources} fontes atingido.</span></div> )}
        ...
        <div className="mt-3 pt-3 border-t border-gray-200 dark:border-[rgba(255,255,255,0.05)] flex-shrink-0">
          {isConfirmingClear ? (
            <div className="text-center">
              ...
              <div className="flex justify-center gap-2">
                <button onClick={() => setIsConfirmingClear(false)} className="px-3 py-1 text-xs ...">Cancelar</button>
                <button onClick={handleConfirmClear} className="px-3 py-1 text-xs ...">Confirmar Limpeza</button>
              </div>
            </div>
          ) : (
            <button onClick={() => setIsConfirmingClear(true)} disabled={isReadingFile || isCreatingGroup} className="w-full flex ...">
              <Trash2 size={14} />
  ...
  return (
    <div className="p-4 bg-white dark:bg-[#1E1E1E] shadow-lg rounded-xl h-full flex flex-col border border-gray-200 dark:border-[rgba(255,255,255,0.05)] transition-colors duration-200">
      <div className="flex items-center justify-between mb-3">
        ...
        {onCloseSidebar && (
          <button onClick={onCloseSidebar} className="p-1 text-gray-500 ... md:hidden">
            <X size={24} />
          </button>
        )}
      </div>
